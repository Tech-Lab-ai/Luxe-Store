...
const CategoryPage: React.FC<Props> = ({ category, onSelectProduct, onNavigate, onSelectCategory, cartCount }) => {
  ...
  // Função para normalizar strings (remover acentos) para comparação robusta
  const normalize = (str: string) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();

  const metadata = useMemo(() => {
    const normalizedCategory = normalize(category);
    return CATEGORIES.find(c => 
      normalize(c.name) === normalizedCategory || 
      normalize(c.slug) === normalizedCategory
    ) || CATEGORIES[0];
  }, [category]);

  const filteredProducts = useMemo(() => {
    const normalizedTarget = normalize(category);
    return MOCK_PRODUCTS.filter(p => 
      (normalize(p.category) === normalizedTarget) &&
  ...
  return (
    <div className="flex flex-col w-full overflow-x-hidden min-h-screen bg-white dark:bg-zinc-950">
      ...
      {/* Hero Dinâmico de Categoria */}
      <section className="px-4 md:px-8 py-10 max-w-7xl mx-auto w-full">
        <div className={`relative h-[300px] md:h-[450px] ${metadata.accentColor || 'bg-zinc-900'} rounded-[48px] overflow-hidden flex items-center px-12 md:px-20 group shadow-2xl`}>
          ...
          <div className="relative z-10 max-w-xl animate-toast">
            <span className="inline-block px-4 py-1.5 bg-brand-primary text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-full mb-6 shadow-lg">
            ...
            <p className="text-zinc-300 font-medium text-lg max-w-md leading-relaxed opacity-90">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
          ...
          <div className="flex items-center gap-6">
             <div className="text-right shrink-0">
                <p className="text-[10px] font-black uppercase text-slate-300 tracking-widest leading-none mb-1">Itens Encontrados</p>
      ...
      <main className="flex-1 px-4 md:px-8 py-16 max-w-7xl mx-auto w-full">
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-40 text-center bg-slate-50/50 dark:bg-zinc-900/30 rounded-[80px] border-2 border-dashed border-slate-100 dark:border-zinc-800 mx-2 animate-toast">
             <div className="w-24 h-24 bg-white dark:bg-zinc-800 rounded-3xl flex items-center justify-center mx-auto mb-10 text-slate-200 shadow-sm">
                <span className="material-icons-round text-6xl">{metadata.icon}</span>
             </div>
             <h3 className="text-3xl font-black mb-4 tracking-tighter uppercase">Coleção em Breve</h3>
             <p className="text-slate-400 max-w-md mx-auto font-medium text-lg leading-relaxed">
                Estamos preparando novidades incríveis para a categoria {metadata.name}. Explore outras coleções enquanto isso!
             </p>
             <button onClick={() => onNavigate(AppView.STOREFRONT)} className="mt-10 px-12 py-5 bg-brand-primary text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-2xl shadow-brand-primary/20 hover:scale-105 active:scale-95 transition-all">Explorar Loja</button>
          </div>
        )}
      </main>

...
const LaptopsPage: React.FC<Props> = ({ onSelectProduct, onNavigate, cartCount }) => {
  const [searchQuery, setSearchQuery] = useState('');
  
  const laptops = MOCK_PRODUCTS.filter(p => 
    p.category === 'Laptops' && 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-zinc-950">
      <Header onNavigate={onNavigate} cartCount={cartCount} onSelectCategory={(cat) => {
        if (cat.toLowerCase() === 'laptops') return;
      }} />
      ...
      <section className="px-4 md:px-8 py-10 max-w-7xl mx-auto w-full">
        <div className="relative h-[300px] md:h-[400px] bg-zinc-900 rounded-[48px] overflow-hidden flex items-center px-12 md:px-20 group">
          <div className="absolute inset-0">
            <img src="https://images.unsplash.com/photo-1517336714731-489689fd1ca8?q=80&w=1600" className="w-full h-full object-cover opacity-30 group-hover:scale-105 transition-transform duration-[5s]" alt="Laptops Banner" />
            <div className="absolute inset-0 bg-gradient-to-r from-black via-black/40 to-transparent"></div>
          </div>
          <div className="relative z-10 max-w-xl">
            <span className="inline-block px-4 py-1.5 bg-brand-primary text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-full mb-6">Workstation & Creative</span>
            <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter leading-none mb-6">Poder sem<br/>Limites<span className="text-brand-primary">.</span></h1>
            <p className="text-zinc-400 font-medium text-lg max-w-md">De máquinas de alta performance a designs ultra-portáteis. Eleve seu fluxo de trabalho para o nível profissional.</p>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 md:px-8 py-8 max-w-7xl mx-auto w-full sticky top-[136px] z-40 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-slate-100 dark:border-zinc-900">
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-400">
             ...
             <span className="text-slate-900 dark:text-white">Laptops</span>
          </div>
          ...
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              placeholder="Buscar modelo ou processador..."
      ...
      <main className="flex-1 px-4 md:px-8 py-16 max-w-7xl mx-auto w-full animate-toast">
        {laptops.length > 0 ? (
          ...
          <div className="py-40 text-center">
            ...
            <h2 className="text-3xl font-black mb-4 tracking-tight">Nenhum laptop encontrado</h2>
            <p className="text-slate-400 font-medium max-w-sm mx-auto">Não encontramos resultados para sua busca de laptops profissionais.</p>
